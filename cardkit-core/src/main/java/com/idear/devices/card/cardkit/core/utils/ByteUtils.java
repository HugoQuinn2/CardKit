...
public abstract class ByteUtils {
    ...
    public static String toBinary(final byte[] pBytes) {
        ...
        if (pBytes != null && pBytes.length > 0) {
            ...
            for (int i = build.length(); i < pBytes.length * BYTE_SIZE; i++) {
                build.insert(0, 0);
            }
            ret = build.toString();
        }
    ...
    /**
     * Method used to convert integer to byte array
     *
     * @param value
    ...
    public static int leastSignificantNibble(byte val) {
        return val & 0x0f;
    }
    ...
    public static byte[] prependByte(byte[] original, byte value) {
